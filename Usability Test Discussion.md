...
1. #### Issues with Colour
    From our user testing, we gained several valuable insights into both the strengths and areas for improvement in our application. One of the most notable findings was the issue with colour choices throughout the interface, particularly in the 'pay up' section. Users found the colour scheme to be washed out, with many expressing a dislike for the predominant blue. This lack of vibrancy affected the visual appeal and contributed to navigation difficulties. Users struggled to distinguish between interactive elements and static content due to low contrast text and a dark navigation bar. These visual design flaws highlighted the need for improvements to enhance usability and accessibility.

2. #### Importance of User Confidence
    Conducting the user testing was an enlightening process, emphasising the critical importance of visual feedback and confirmation after performing an action. For instance, the 'pay up' feature lacked immediate, clear confirmation, which left users uncertain if their actions were successfully completed. Feedback from the application must be immediate and unambiguous to foster user confidence and satisfaction. Moreover, our users' struggles with navigation underscored the need for a more intuitive and visually engaging interface. Ensuring that interactive elements are clearly distinguishable and easily accessible is essential for a positive user experience.

3. #### Skewing Sentiment Analysis
    Interestingly, we observed how certain tasks could inadvertently affect our sentiment analysis. In one particular task, users were instructed to remove an expense labelled "garbage." This specific instruction led to numerous instances of users verbally saying "garbage," which in turn skewed our sentiment analysis results negatively. This highlighted the importance of task design in user testing, as seemingly minor details can significantly impact the data collected. To mitigate such issues, we need to carefully design tasks to avoid unintended biases and ensure accurate sentiment analysis.

4. #### Documentation of Issues
    To systematically address these findings, we documented them in our GitLab issues. Specific issues included changing the colours in the navigation bar and heading (#8), making the navigation bar more visually obvious (#13), and improving the visibility of certain text in dark mode (#3). Although we did not initially consistently use Material Design guidelines during the design phase, incorporating them now could enhance our application's usability by improving colour contrast and visual clarity. These changes are not just aesthetic but functional, aiding in clearer communication and easier navigation. By addressing these issues and considering **Material Design principles**, we aim to create a more user-friendly and visually appealing application. This approach will help ensure that our application meets user needs more effectively and delivers a more satisfying user experience.
...
1. #### User Testing Organisation - Time Complexity and Clarity
    ...
    On the plus side, one thing we did do quite well was time management. Our group was quick to set up our testing plan and schedule a meeting with our testers, enabling us to make good progress and analyse our testing results well ahead of schedule. We then managed our data analysis efficiently to ensure all our members had adequate time to write our reports.
